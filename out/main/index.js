...
let win;
const iconPath = path$1.join(__dirname, "../../public/music.png");
function createWindow() {
  win = new BrowserWindow({
    ...
    // 去掉窗口边框
    webPreferences: {
      preload: path$1.join(__dirname, "../preload/index.mjs"),
  ...
  if (!app.isPackaged) {
    ...
    win.loadFile(path$1.join(__dirname, "../../out/renderer/index.html"));
  }
}
  ...
  try {
    ...
    if (metadata.common.lyrics) {
      if (Array.isArray(metadata.common.lyrics)) {
        ...
        const lyricsEntries = Object.entries(metadata.common.lyrics).sort(([langA], [langB]) => (langA || "").localeCompare(langB));
        return lyricsEntries.map(([_2, text]) => text).join("\n");
  ...
  try {
    ...
    if (searchResponse.data?.result?.songs && searchResponse.data.result.songs.length > 0) {
      ...
      for (const song of songs) {
        const songArtists = song.artists?.map((artist2) => artist2.name).join(" ") || "";
        if (songArtists.toLowerCase().includes(artist.toLowerCase()) || artist.toLowerCase().includes(songArtists.toLowerCase())) {
          bestMatch = song;
          break;
        }
      }
...
async function fetchLyricsFromQQMusic(title, artist) {
  try {
    const searchQuery = encodeURIComponent(`${title} ${artist}`);
...
ipcMain.handle("parse-music-metadata", async (_, filePath) => {
  try {
    const metadata = await parseFile(filePath);
    const { common: common2, format } = metadata;
    let albumArt = null;
    if (common2.picture && common2.picture.length > 0) {
      const picture = common2.picture[0];
    ...
    return {
      title: common2.title || "未知标题",
      artist: common2.artist || "未知艺术家",
      album: common2.album || "未知专辑",
      duration: format.duration || 0,
      albumArt,
      genre: common2.genre ? common2.genre.join(", ") : "未知流派",
      year: common2.year || null
    };
  } catch (error) {
